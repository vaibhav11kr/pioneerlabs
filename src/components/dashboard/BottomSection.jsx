...
const BottomSection = () => {
  ...
  const fetchData = async () => {
    ...
    setData({
      ...
      setData(Object.values(fetchedData.bpi));
      ...
      <div className="flex flex-row gap-4 overflow-scroll w-full">
        {!loading ?
          Array.isArray(data) && data.map((item) => (
            ...
            <div className="bg-[#171717] w-48 rounded-lg p-2" key={item.code}>
              ...
              <div className="flex flex-row justify-between items-center mb-3">
                <p className={`text-lg ${item.symbol}`}>{item.rate} {item.symbol}</p>
                <p className="text-[10px] text-[#4F4F4F] ">
  {item.rate_float < 0 ? ('+' + item.rate_float ): item.rate_float} 
</p>
              </div>
